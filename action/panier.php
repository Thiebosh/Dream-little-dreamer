<?php
if (isset($variablePage['postPanier'])) {
    $postSecure = $variablePage['postPanier'];
}
...
if (!empty($postSecure['supprimePanier'])) {
    ...
    if (isset($postSecure['ref'], $postSecure['article'], $postSecure['prix'], $postSecure['quant'], $postSecure['dispo'])) {
        ...
        if (!empty($_SESSION['panier']) && array_key_exists($postSecure['article'], $_SESSION['panier'])) {
            $_SESSION['panier'][$postSecure['article']]['quantite'] = $postSecure['quant'];
        }

        //sinon, crée nouvelle entrée du nom de l'article
        else {
            $_SESSION['panier'][$postSecure['article']] = array('id_produit'   => $postSecure['ref'],
    ...
    else if (!empty($postSecure['supprimeArticle'])) {
        unset($_SESSION['panier'][$postSecure['supprimeArticle']]);
    }
