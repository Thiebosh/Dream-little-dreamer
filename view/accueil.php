<?php ob_start(); ?>
    <section id="pageAccueil">
        <h2> Notre nouvelle boutique <em>Dream Little dreamer</em> vient d'ouvrir !</h2>
        <p> Sur ce site, vous pourrez trouver tous les objets de décoration qui vous permettront de créer votre propre univers...</p>
        
        <article class="slider-holder">
            <span id="slider-image-1"></span>
            <span id="slider-image-2"></span>
            <span id="slider-image-3"></span>
            <div class="image-holder">
                <img class="slider-image" src="view/images/deco.jpg" alt="Visuel 1"/>
                <img class="slider-image" src="view/images/lit.jpg"  alt="Visuel 2"/>
                <img class="slider-image" src="view/images/room.jpg" alt="Visuel 3"/>
            </div>
            <div class="button-holder">
                <a href="#slider-image-1" class="slider-change"></a>
                <a href="#slider-image-2" class="slider-change"></a>
                <a href="#slider-image-3" class="slider-change"></a>
            </div>
        </article>
    </section>
<?php $variablePage['contenuSection'] = ob_get_clean();

require("template.php");

